...
export type QuestionItem = {
  ...
  required: boolean;
  options: string[];
  update: (prev: QuestionActionState, formData: FormData) => Promise<QuestionActionState>;
  remove: () => Promise<void>;
  moveUp: () => Promise<void>;
...
function QuestionFields({ defaults }: { defaults?: Partial<QuestionItem> }) {
  const [fieldType, setFieldType] = React.useState(defaults?.fieldType ?? "text");
  return (
    <div className="space-y-3">
      <div>
        <Label>Question label</Label>
        <Input name="label" required defaultValue={defaults?.label ?? ""} placeholder="T-shirt size" />
      </div>
...
function EditForm({ question, onDone }: { question: QuestionItem; onDone: () => void }) {
  const [state, formAction, pending] = useActionState(
    async (prev: QuestionActionState, formData: FormData) => {
...
export function QuestionsEditor({
  ...
  createAction: (prev: QuestionActionState, formData: FormData) => Promise<QuestionActionState>;
}) {
    ...
    <div className="space-y-4">
      {questions.length === 0 && !showAdd ? (
        <EmptyState
          ...
          hint="Ask guests anything extra on the RSVP form, like meal choice or t-shirt size."
